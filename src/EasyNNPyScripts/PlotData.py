...
def PlotHypothesis(ax, x1_coords, x2_coords, thetas, y_values, plotColor='blue', plotAlpha=0.3, boundary = 3):
    x1_range = np.arange(min(x1_coords)-boundary, max(x1_coords)+boundary, 1)
    x2_range = np.arange(min(x2_coords)-boundary, max(x2_coords)+boundary, 1)
...
def CompareHypothesis(X, y, easyNNTheta, tensorFlowTheta):
    ...
    PlotHypothesis(ax, [row[0] for row in X], [row[1] for row in X], tensorFlowTheta, y, plotColor='blue', boundary = 2)
